...
3. Choose **Inspect content**. Review the decision, evidence, safe handoff, and inspection stages.
...
The response includes `decision` (`allow`, `sanitize`, or `quarantine`), `risk`, `modelScore`, `findings`, `sanitizedText`, `safeHandoff`, and a stage-by-stage trace. The caller must pass only `safeHandoff` to its downstream model when it is non-null. `GET /api/evaluate` runs the curated fixture checks. Source labels must be set by the trusted integration, not accepted from untrusted retrieved content.
