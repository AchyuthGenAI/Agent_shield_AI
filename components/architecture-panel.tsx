...
const steps = [
  ...
  { icon: LockKeyhole, title: "5. Handoff & demo", body: "Only the approved handoff feeds a fixed extractive briefing. Quarantined input gives the demo nothing to read." },
];
...
export function ArchitecturePanel() {
  return <section className="detail-surface" aria-labelledby="architecture-heading">
    ...
    <div className="architecture-bottom"><div><h3>Use it in an agent pipeline</h3><p>The server inspects each request, then passes only <code>safeHandoff</code> to its deterministic demo. A real agent integration should use the same boundary and keep tool permissions separate.</p></div><pre>{"POST /api/inspect\nContent-Type: application/json\n\n{\"source\":\"web\",\"content\":\"...\"}\n\n→ decision: allow | sanitize | quarantine\n→ findings, risk, safeHandoff, stages\n→ protectedAgent: extractive demo result"}</pre></div>
    <p className="architecture-note">The protected step is an extractive simulation with no language model or tools. The small local classifier and rules can miss novel attacks; a production agent still needs independent evaluation and tool authorization.</p>
  </section>;
}
